...
func (s *Storage) CreateContact(ctx context.Context, c *models.Contact) error {
	query := `
		INSERT INTO contacts (user_id, channel_id, platform, platform_user_id, name, phone, email, budget, preferred_location, purchase_timeline, tags, is_hot_lead, created_at, updated_at)
...
func (s *Storage) GetOrCreateContact(ctx context.Context, c *models.Contact) error {
	query := `
		SELECT id, name, phone, email, budget, preferred_location, purchase_timeline, tags, is_hot_lead, created_at, updated_at
		FROM contacts
	...
	err := s.DB.QueryRow(ctx, query, c.UserID, c.Platform, c.PlatformUserID).Scan(
		...
		&c.PreferredLocation, &c.PurchaseTimeline, &c.Tags,
		&c.IsHotLead, &c.CreatedAt, &c.UpdatedAt,
	)
	if err != nil {
...
func (s *Storage) GetContactsByUser(ctx context.Context, userID int64, limit, offset int) ([]models.Contact, error) {
	query := `
		SELECT id, user_id, channel_id, platform, platform_user_id, name, phone, email,
		       budget, preferred_location, purchase_timeline, tags, is_hot_lead, created_at, updated_at
		FROM contacts
		WHERE user_id = $1
